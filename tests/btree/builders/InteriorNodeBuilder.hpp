#ifndef MKVDB_TESTS_BTREE_BUILDERS_INTERIOR_NODE_BUILDER_HPP_
#define MKVDB_TESTS_BTREE_BUILDERS_INTERIOR_NODE_BUILDER_HPP_
...
namespace mkvdb::tests::btree::builders
{
    ...
    public:
        ...
        InteriorNodeBuilder& WithItemsCount(std::uint32_t items_count)
        {
            items_count_ = items_count;
            return *this;
        }

        InteriorNodeBuilder& WithKeySize(mkvdb::common::KeyValueSize key_size)
        {
            key_size_ = key_size;
            return *this;
        }

        InteriorNodeBuilder& WithValueSize(mkvdb::common::KeyValueSize value_size)
        {
            value_size_ = value_size;
            return *this;
        }

        InteriorNodeBuilder& WithPageSize(mkvdb::pager::Page::PageSize page_size)
        {
            page_size_ = page_size;
            return *this;
        }

        InteriorNodeBuilder& WithPageIndex(mkvdb::pager::Page::PageIndex page_index)
        {
            page_index_ = page_index;
            return *this;
        }

        mkvdb::btree::InteriorNode& Build()
        {
            if(!node_)
            {
                btree_.emplace(key_size_, value_size_);
                page_ = std::make_shared<mkvdb::pager::Page>(page_index_, page_size_);
                node_.emplace(*btree_, page_, true);
    ...
    private:
        ...
        std::optional<mkvdb::btree::InteriorNode> node_;
    };

} // namespace mkvdb::tests::btree::builders

#endif // MKVDB_TESTS_BTREE_BUILDERS_INTERIOR_NODE_BUILDER_HPP_